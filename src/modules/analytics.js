...
export async function initAnalytics(container, navigateFn, storeId = null) {
 ...
 All Time
  ...
  <div class="card bg-zinc-950 text-white relative overflow-hidden group">
  <div class="absolute top-0 right-0 p-3 opacity-10">
  <i data-lucide="infinity" class="w-8 h-8"></i>
  </div>
  <div class="card-header pb-2">
  <h3 class="card-title text-zinc-400 text-xs font-semibold uppercase tracking-wider">Lifetime Revenue</h3>
  </div>
  ...
  <!-- Sales -->
  <div class="card group hover:border-primary/50 transition-colors">
  <div class="card-header pb-2 flex flex-row items-center justify-between space-y-0 relative">
  <h3 class="card-title text-sm font-medium text-muted-foreground" id="label-sales">Sales (All Time)</h3>
  <i data-lucide="trending-up" class="w-4 h-4 text-muted-foreground opacity-50 absolute right-6 top-6"></i>
  </div>
  <div class="card-content">
  <p class="text-2xl font-bold" id="month-sales-card">₹0</p>
  <p class="text-xs text-muted-foreground mt-1 flex items-center"><span class="w-2 h-2 rounded-full bg-primary mr-2"></span> Revenue</p>
  </div>
  </div>
  
  <!-- Expenditure -->
  <div class="card group hover:border-destructive/50 transition-colors">
  <div class="card-header pb-2 flex flex-row items-center justify-between space-y-0 relative">
  <h3 class="card-title text-sm font-medium text-muted-foreground" id="label-exp">Expenditure (All Time)</h3>
  <i data-lucide="wallet" class="w-4 h-4 text-muted-foreground opacity-50 absolute right-6 top-6"></i>
  </div>
  <div class="card-content">
  <p class="text-2xl font-bold" id="month-expenditure-card">₹0</p>
  <p class="text-xs text-muted-foreground mt-1 flex items-center"><span class="w-2 h-2 rounded-full bg-destructive mr-2"></span> Expenses</p>
  </div>
  </div>

  <!-- Net Profit -->
  <div class="card group hover:border-emerald-500/50 transition-colors">
  <div class="card-header pb-2 flex flex-row items-center justify-between space-y-0 relative">
  <h3 class="card-title text-sm font-medium text-muted-foreground" id="label-profit">Net Profit (All Time)</h3>
  <i data-lucide="piggy-bank" class="w-4 h-4 text-muted-foreground opacity-50 absolute right-6 top-6"></i>
  </div>
  <div class="card-content">
  <p class="text-2xl font-bold" id="net-profit-card">₹0</p>
  <div class="mt-2 text-xs text-muted-foreground flex flex-col gap-1">
  <div class="flex justify-between"><span>COGS:</span> <span id="cogs-display" class="text-destructive font-medium">₹0</span></div>
  <div class="flex justify-between items-center bg-muted/50 p-1 rounded">
  <span>Sales - COGS - Exp</span>
  <button id="download-financial-report" class="text-muted-foreground hover:text-primary transition-colors p-1" title="Download Report">
  ...
  <!-- Total COGS -->
  <div class="card group hover:border-orange-500/50 transition-colors">
  <div class="card-header pb-2 flex flex-row items-center justify-between space-y-0 relative">
  <h3 class="card-title text-sm font-medium text-muted-foreground">Total COGS</h3>
  <i data-lucide="tag" class="w-4 h-4 text-muted-foreground opacity-50 absolute right-6 top-6"></i>
  </div>
  <div class="card-content">
  <p class="text-2xl font-bold" id="total-cogs-card">₹0</p>
  <p class="text-xs text-muted-foreground mt-1 flex items-center"><span class="w-2 h-2 rounded-full bg-orange-500 mr-2"></span> Cost of Goods</p>
  </div>
  </div>

  <!-- Active Repairs -->
  <div class="card group hover:border-violet-500/50 transition-colors">
  <div class="card-header pb-2 flex flex-row items-center justify-between space-y-0 relative">
  <h3 class="card-title text-sm font-medium text-muted-foreground">Active Repairs</h3>
  <i data-lucide="wrench" class="w-4 h-4 text-muted-foreground opacity-50 absolute right-6 top-6"></i>
  </div>
  <div class="card-content">
  <p class="text-2xl font-bold" id="active-repairs">0</p>
  <p class="text-xs text-muted-foreground mt-1 flex items-center"><span class="w-2 h-2 rounded-full bg-violet-500 mr-2"></span> In Progress</p>
  </div>
  </div>
  </div>
 ...
 <div class="card-content h-[300px] flex justify-center items-center">
 ...
 <button id="view-all-txns" class="text-xs font-medium text-primary hover:underline transition-colors mt-0">View All</button>
 </div>
 <div class="card-content flex-1 overflow-auto max-h-[300px]">
 <table class="table">
 ...
 <tbody id="recent-txns" class="table-body"></tbody>
 </table>
 </div>
 </div>
 ...
 <button id="view-all-exp" class="text-xs font-medium text-primary hover:underline transition-colors mt-0">View All</button>
 </div>
 <div class="card-content flex-1 overflow-auto max-h-[300px]">
 <table class="table">
 <thead class="table-header sticky top-0 bg-card z-10">
 ...
 <tbody id="recent-exp" class="table-body"></tbody>
 </table>
 </div>
 </div>
 ...
 <button id="view-all-queries" class="text-xs font-medium text-primary hover:underline transition-colors mt-0">View All</button>
 </div>
 <div class="card-content flex-1 overflow-auto max-h-[300px]">
 <table class="table">
 <thead class="table-header sticky top-0 bg-card z-10">
 <tr>
 <th class="table-head">Customer / Req</th>
 <th class="table-head text-right">Status</th>
 </tr>
 </thead>
 <tbody id="recent-queries" class="table-body"></tbody>
 </table>
 </div>
 </div>
 </div>
 ...
 <tr class="table-row">
 <td class="table-cell">
 <div class="font-medium truncate max-w-[120px]">${b.customer_name || 'Walk-in'}</div>
 <div class="text-[10px] text-muted-foreground font-mono">#${b.invoice_number || b.id.slice(0, 6)} • ${new Date(b.created_at).toLocaleDateString()}</div>
 </td>
 <td class="table-cell text-right">
 <span class="font-semibold text-foreground">₹${b.total_amount}</span>
 </td>
 </tr>
 `).join('') || '<tr><td colspan="2" class="table-cell text-center text-muted-foreground text-xs py-4">No recent transactions</td></tr>';
 ...
 <tr class="table-row">
 <td class="table-cell">
 <div class="font-medium truncate max-w-[120px]">${e.item_name}</div>
 <div class="text-[10px] text-muted-foreground">${e.category || 'General'} • ${new Date(e.created_at).toLocaleDateString()}</div>
 </td>
 <td class="table-cell text-right">
 <span class="font-semibold text-destructive">-₹${e.amount}</span>
 </td>
 </tr>
 `).join('') || '<tr><td colspan="2" class="table-cell text-center text-muted-foreground text-xs py-4">No recent expenditures</td></tr>';
 ...
 <tr class="table-row">
 <td class="table-cell">
 <div class="font-medium truncate max-w-[120px]" title="${q.customer_name}">${q.customer_name}</div>
 <div class="text-[10px] text-muted-foreground truncate max-w-[120px]" title="${q.requirement}">${q.requirement}</div>
 </td>
 <td class="table-cell text-right">
 <span class="badge ${getStatusColor(q.status)}">
 ${q.status}
 </span>
 </td>
 </tr>
 `).join('') || '<tr><td colspan="2" class="table-cell text-center text-muted-foreground text-xs py-4">No recent queries</td></tr>';
 }

 function getStatusColor(status) {
 switch (status?.toLowerCase()) {
 case 'pending': return 'badge-secondary text-yellow-600 bg-yellow-50 hover:bg-yellow-100';
 case 'resolved': return 'badge-secondary text-emerald-600 bg-emerald-50 hover:bg-emerald-100';
 case 'in progress': return 'badge-secondary text-blue-600 bg-blue-50 hover:bg-blue-100';
