...
export async function initBilling(container, storeId = null) {
 ...
 <div>
 <h2 class="text-2xl font-bold tracking-tight">New Bill</h2>
 <p class="text-muted-foreground">Select products, repairs, or add custom items</p>
 </div>
 
 <div class="card flex-1 flex flex-col overflow-y-auto">
 ...
 <div class="card h-full flex flex-col relative"> 
 <div class="card-header flex flex-row justify-between items-start pb-4 border-b">
 <div>
 <h3 class="card-title">Invoice Draft</h3>
 <p class="text-xs text-muted-foreground mt-1" id="invoice-date">${new Date().toLocaleDateString()}</p>
 </div>
 <div class="flex items-center gap-3">
 <select id="payment-status" class="input-field h-10 text-xs py-0 pl-3 pr-8 w-36 py-2">
 <option value="Paid">Paid</option>
 <option value="Pending">Pending</option>
 </select>
 
 <!-- GST Controls -->
 <div class="flex items-center gap-2 bg-muted p-1 rounded-md border">
 <label class="inline-flex items-center cursor-pointer mr-1">
 <input type="checkbox" id="gst-toggle" class="sr-only peer">
 <span class="px-2 py-1 text-xs font-bold text-muted-foreground peer-checked:text-foreground transition-colors">GST</span>
 </label>
 <select id="gst-type-select" class="h-6 text-[10px] py-0 pl-2 pr-6 border-none bg-transparent text-foreground font-medium focus:ring-0 hidden" disabled>
 ...
 <div class="card-content flex-1 flex flex-col pt-4">
 ...
 <tr>
 <th class="table-head w-1/3">Item</th>
 <th class="table-head w-1/4">Details</th>
 <th class="table-head text-center">Qty</th>
 <th class="table-head text-right">Price</th>
 <th class="table-head text-right">Total</th>
 <th class="table-head"></th>
 </tr>
 </thead>
 <tbody id="cart-items" class="table-body">
 <!-- Cart Items -->
 </tbody>
 </table>
 <div id="empty-cart-msg" class="text-center py-10 text-muted-foreground text-sm italic">
 No items added yet.
 </div>
 </div>

 <div class="bg-muted/30 rounded-xl p-4 mt-4 space-y-2 border">
 <div class="flex justify-between text-muted-foreground">
 <span>Subtotal</span>
 <span id="subtotal-display">₹0.00</span>
 </div>
 <div class="flex justify-between text-muted-foreground hidden gst-line" id="cgst-row">
 <span>CGST (9%)</span>
 <span id="cgst-display">₹0.00</span>
 </div>
 <div class="flex justify-between text-muted-foreground hidden gst-line" id="sgst-row">
 <span>SGST (9%)</span>
 <span id="sgst-display">₹0.00</span>
 </div>
 <div class="flex justify-between text-muted-foreground hidden gst-line" id="igst-row">
 <span>IGST (18%)</span>
 <span id="igst-display">₹0.00</span>
 </div>
 <div class="flex justify-between text-xl font-bold text-foreground pt-2 border-t mt-2">
 <span>Total</span>
 <span id="total-display">₹0.00</span>
 </div>
 </div>

 <button id="generate-bill-btn" class="btn-primary w-full mt-4 py-3 text-lg" disabled>
 Generate Invoice
 </button>
 ...
 <td class="table-cell align-top font-medium">
 ${item.name} 
 ${item.isManual ? '<span class="badge badge-secondary ml-1">MANUAL</span>' : ''}
 </td>
 <td class="table-cell align-top">
 ${details}
 </td>
 <td class="table-cell text-center align-top">${item.qty}</td>
 <td class="table-cell text-right align-top">₹${item.price.toFixed(2)}</td>
 <td class="table-cell text-right font-medium align-top">₹${(item.price * item.qty).toFixed(2)}</td>
 <td class="table-cell text-right opacity-0 group-hover:opacity-100 transition-opacity align-top">
 <button class="cart-remove-btn text-muted-foreground hover:text-destructive transition-colors" data-idx="${idx}">
