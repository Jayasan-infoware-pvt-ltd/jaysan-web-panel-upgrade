...
export async function initStock(container, storeId = null) {
  container.innerHTML = `
        <div class="flex items-center justify-between space-y-2 mb-6">
            <div>
                <h2 class="text-3xl font-bold tracking-tight">Stock Inventory</h2>
                <p class="text-muted-foreground">Manage your product catalog, pricing, and inventory levels.</p>
            </div>
            <div class="flex items-center space-x-2">
                <button id="export-stock-btn" class="btn-outline">
        ...
        <div class="card mb-6">
            <div class="card-content flex items-center space-x-4 pt-6">
                <div class="relative flex-1">
                    <i data-lucide="search" class="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground"></i>
                    <input type="text" id="search-stock" placeholder="Search products..." class="input-field pl-9" />
                </div>
                <div class="w-[200px]">
            ...
            <div class="table-wrapper">
                <table class="table">
                    <thead class="table-header">
                        <tr>
                            <th class="table-head w-24">Img</th>
                            ...
                            <th class="table-head w-[80px]"></th>
                        </tr>
                    </thead>
                    <tbody id="stock-table-body" class="table-body">
                        <tr><td colspan="8" class="table-cell text-center py-10 text-muted-foreground">Loading...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
 ...
 <!-- Modal Template -->
 <div id="product-modal" class="dialog-overlay hidden">
 <div class="dialog-content">
 <div class="dialog-header mb-4 border-b pb-4">
 <h3 id="modal-title" class="dialog-title">Add Product</h3>
 <p class="dialog-description">Fill out the details below to add a new product to inventory.</p>
 </div>
 <form id="product-form" class="space-y-4 max-h-[60vh] overflow-y-auto px-1 custom-scrollbar">
 ...
 <label class="mt-2 flex items-center justify-center w-full h-12 border border-dashed rounded-md hover:bg-muted cursor-pointer transition-colors">
 <i data-lucide="plus" class="w-4 h-4 text-muted-foreground mr-2"></i>
 <span class="text-sm text-muted-foreground">Add Images</span>
 <input type="file" id="product-image-input" accept="image/*" multiple class="hidden">
 </label>
 <button type="button" id="add-image-url-btn" class="mt-2 flex items-center justify-center w-full h-10 border border-dashed rounded-md hover:bg-muted text-muted-foreground transition-colors">
 <i data-lucide="link" class="w-4 h-4 mr-2"></i>
 <span class="text-sm">Add via URL</span>
 </button>
 </div>
 ...
 <div class="bg-muted p-4 rounded-md border">
 <div class="flex justify-between items-center mb-2">
 <label class="text-sm font-semibold">Actual Price (Cost)</label>
 <button type="button" id="unlock-cost-btn" class="text-xs text-primary hover:underline flex items-center">
 <i data-lucide="lock" class="w-3 h-3 mr-1"></i>Unlock
 </button>
 </div>
 <div class="relative">
 <input type="number" id="product-cost" class="input-field bg-background" placeholder="Locked" disabled>
 <div id="cost-mask" class="absolute inset-0 bg-background/90 backdrop-blur-sm flex items-center justify-center text-xs text-muted-foreground font-mono select-none rounded-md">
 ••••••••
 </div>
 </div>
 <p class="text-[10px] text-muted-foreground mt-2">Hidden from standard users. Required for profit calculation.</p>
 </div>

 <!-- Dynamic Serials Container -->
 <div class="bg-muted/50 p-4 rounded-md border">
 <label class="label text-muted-foreground uppercase text-xs">Serial Numbers</label>
 <div id="serials-container" class="space-y-2 max-h-40 overflow-y-auto pr-1">
 <p class="text-xs text-muted-foreground italic">Enter quantity to add serial numbers.</p>
 </div>
 </div>
 ...
 <div id="product-meta-panel" class="hidden mt-4 pt-4 border-t space-y-4">
 <div class="grid grid-cols-2 gap-4 text-sm">
 <div class="flex flex-col space-y-1">
 <span class="text-xs font-medium text-muted-foreground uppercase">Created</span>
 <div id="meta-created" class="font-medium">—</div>
 </div>
 <div class="flex flex-col space-y-1">
 <span class="text-xs font-medium text-muted-foreground uppercase">Last Updated</span>
 <div id="meta-updated" class="font-medium">—</div>
 </div>
 </div>
 <div class="flex flex-col space-y-2">
 <div class="flex items-center justify-between">
 <span class="text-xs font-medium text-muted-foreground uppercase">Update History</span>
 </div>
 <div id="product-history-list" class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar text-sm">
 <div class="text-muted-foreground text-sm italic py-2">Loading history...</div>
 </div>
 </div>
 </div>
 </div>
 ...
 if (items.length === 0) {
 tbody.innerHTML = `<tr><td colspan="8" class="table-cell text-center text-muted-foreground py-10">No products found</td></tr>`;
 return;
 }
 ...
 <img src="${thumb}" class="w-12 h-12 rounded-md object-cover border" alt="${p.name}">
 ...
 `<div class="w-12 h-12 rounded-md bg-muted flex items-center justify-center text-muted-foreground"><i data-lucide="image" class="w-5 h-5"></i></div>`
 }
 </td>
 <td class="table-cell font-medium">${p.name}</td>
 <td class="table-cell">
 <span class="badge badge-outline ${getCategoryColor(p.category)}">
 ${p.category || 'General'}
 </span>
 </td>
 <td class="table-cell text-right">₹${p.price}</td>
 <td class="table-cell text-right">
 <span class="${p.quantity < 5 ? 'text-destructive font-bold' : 'text-emerald-600 font-medium'}">
 ${p.quantity}
 </span>
 </td>
 ...
 <td class="table-cell text-right">
 <button class="menu-trigger btn-icon rounded-full hover:bg-muted text-muted-foreground" data-id="${p.id}">
